...
@router.get('/list')
def list_songs(db: Session=Depends(get_db), auth_dist=Depends(auth_middleware)):
      songs = db.query(Song).all()
      return songs

@router.post('/favorite')
def favorite_song(song: FavoriteSong, db: Session=Depends(get_db), auth_dist=Depends(auth_middleware)):
      # song is already favorited by the user
      user_id = auth_dist['uid']
...
def list_fav_songs(db: Session=Depends(get_db), auth_dist=Depends(auth_middleware)):
      user_id = auth_dist['uid']
